# -*- coding: utf-8 -*-
"""
Created on Tue Dec  1 22:20:25 2020
...
random.seed(2) #set seed for random generation
...
def simulate_interference(wave_data, data, dc_offset = [0,0,0]):
    ...
    """
    modified_data = []
    time = np.array([i for i in range(len(data[0]))])*time_interval
    sine_wave = add_sine_wave(time)
    for direction in range(3):
        dir_data = data[direction]
        temp_data = dir_data
        temp_data = temp_data + dc_offset[direction]
        temp_data = temp_data + sine_wave
...
def gen_wave_data(n, max_amp, max_freq):
    ...
    wave_data = [(1,0,0)]
    major_wave = (random.uniform(0.1,1.2), 0, random.uniform(3.5,5))
    wave_data.append(major_wave)
    minor_wave = (random.uniform(0.1,1.2), 0, random.uniform(1.5,3.5))
    wave_data.append(minor_wave)
    for i in range(n-2):
        temp_wave = (random.uniform(0.04,1), 0, random.uniform(0.25,1.5))
        wave_data.append(temp_wave)
    return wave_data

def add_sine_wave(time, max_amp = 10, max_freq = 30, randomf=True):
    ...
    if randomf:
        amp = random.uniform(5, 10)#max_amp * np.random.random(size=None)
        freq = random.uniform(1, 20)
    else:
        amp = max_amp
        freq = max_freq
    sine_wave = amp * np.sin(2 * np.pi * freq * time)
    print("half of sin amp:", 0.5*amp)
    return sine_wave
...
def main(PATH):
    ...
    for i in range(0,7):
        wave_data = gen_wave_data(8, 5, 1.5)
        dc = dc_interference() #generate consistent dc interference
        avg_mag = 0
    
        out_data = simulate_interference(wave_data[:6], data, dc) #only use i waves in the interference
        for wave in wave_data:
            avg_mag += wave[2]*0.5
        print(avg_mag)
        outfile = PATH[:9] + "_modified_"+str(i)+".txt"
        save(out_data, outfile)
        file_names.append(PATH[:9]+ "_modified_"+str(i)+".txt")
        
    f = open("temp_file_list.txt", "a")
    for i in range(len(file_names)):
        f.write(file_names[i] + "\n")


main("C1_160309_cavity.txt")
print("done")
main("C2_160313_cavity.txt")
print("done")
main("C3_160318_cavity.txt")
print("done")
main("C4_160318_cavity.txt")